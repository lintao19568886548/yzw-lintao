...
pub(crate) fn DatePickerInput(props: DatePickerInputProps) -> Element {
    ...
    rsx! {
        date_picker::DatePickerInput {
            ...
            // 上游默认居中对齐（align=center），日历比输入框宽，会往两侧各溢出一截。
            // 放进弹窗里，弹窗为了限高开了 overflow，左侧那截就被直接裁掉。
            // 改成左对齐触发器，和 Select 的下拉列表（left: 0）保持一致。
            DatePickerPopoverContent { align: ContentAlign::Start,
