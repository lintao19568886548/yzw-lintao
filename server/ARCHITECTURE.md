...
4. **已由最新决策替代：**生产工作流和远端脚本永久移除清库变量、重建和任意发布命令入口；不再把“变量保持关闭”作为数据保护措施。
...
生产工作流已在 P0-01 改为仅允许手动触发；每次申请生产发布前必须完成 Schema 影响评估：
...
3. 生产工作流与远端脚本是否仍通过 P0-01 破坏性入口扫描，且不存在清库、重建、强制发布或任意命令入口；
